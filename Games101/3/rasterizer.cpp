...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (int j = lower_y; j < upper_y; j++) {
        for (int i = lower_x; i < upper_x; i++) {
            ...
            weight = weight * w_reciprocal;

            float z_interpolated = v[0].z() * weight.x() + v[1].z() * weight.y() + v[2].z() * weight.z();
            if (z_interpolated <= depth_buf[get_index(i, j)]) {
                Eigen::Vector3f color = t.color[0] * weight.x() + t.color[1] * weight.y() + t.color[2] * weight.z();
                Eigen::Vector3f normal = t.normal[0] * weight.x() + t.normal[1] * weight.y() + t.normal[2] * weight.z();
                Eigen::Vector2f tex_coords = t.tex_coords[0] * weight.x() + t.tex_coords[1] * weight.y() + t.tex_coords[2] * weight.z();
